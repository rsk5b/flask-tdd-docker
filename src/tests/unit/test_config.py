...
def test_local_config(test_app):
    test_app.config.from_object('src.config.LocalConfig')
    assert test_app.config['SECRET_KEY'] == 'my_precious'
    assert test_app.config['TESTING']
    assert test_app.config['SQLALCHEMY_DATABASE_URI'] == 'postgresql://postgres:postgres@localhost:5432/api_dev'
